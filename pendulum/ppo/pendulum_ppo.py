...
class PerformanceLogger:
    ...
    def __init__(self, save_dir="./pendulum/ppo/json_data/"):
    ...
    def log_episode(self, episode_num, reward, episode_length, training_time, 
        ...
        # Log PPO-specific losses if provided
    ...
    def save_data(self, filename_prefix="ppo_pendulum"):
...
class TrainingCallback(BaseCallback):
    ...
    def _on_episode_end(self):
        ...
        if self.episode_start_time is not None:
            ...
            # Try to get PPO-specific loss information from model
            policy_loss = None
            value_loss = None
            entropy_loss = None
            clip_fraction = None
...
def run(is_training=True):
    print("Setting up Pendulum with PPO...")
    ...
    # Create directories
    os.makedirs("./pendulum/ppo/weights/", exist_ok=True)
    os.makedirs("./pendulum/ppo/json_data/", exist_ok=True)
    ...
    if is_training:
        ...
        # PPO configuration optimized for Pendulum continuous control
        model = PPO(
            "MlpPolicy",
            env,
            ...
            ent_coef=0.0,                    # No entropy regularization (dense rewards)
        ...
        print("Model created with PPO configuration for Pendulum:")
        ...
        print(f"- Gamma: 0.99 (standard discount factor)")
        print(f"- Clip Range: 0.2 (PPO clipping parameter)")
        print(f"- Entropy Coefficient: 0.0 (dense rewards, no exploration bonus)")
        ...
        print(f"- Action Space: Continuous torque control")
        
        # Create callback for logging
        callback = TrainingCallback(logger, log_interval=2000)
        
        print("\nStarting training...")
        print("Note: PPO should be very stable and sample efficient for Pendulum")
        print("Expected smooth learning curve with consistent improvement")
        
        # Train the model (PPO typically needs fewer steps than A2C)
        total_timesteps = 800000  # 80k steps should be sufficient for PPO on Pendulum
        ...
        # Save the model
        model.save("./pendulum/ppo/weights/ppo_pendulum")
        ...
        if logger.episode_rewards:
            ...
            plt.tight_layout()
            plt.savefig("./pendulum/ppo/weights/ppo_pendulum_training.png", 
                       dpi=300, bbox_inches='tight')
            plt.show()
            
            # Print final training summary
            final_metrics = logger.get_performance_metrics()
            print("\n=== PPO PENDULUM TRAINING SUMMARY ===")
            ...
            print(f"Dense reward environment - should see smooth learning")
            print(f"Continuous control challenge - balancing pendulum upright")
            ...
            if logger.clip_fractions:
                ...
                if avg_clip_fraction < 0.05:
                    print("Low clip fraction suggests conservative policy updates")
                ...
                print(f"PPO clipping prevents destructive policy updates")
                print(f"Multi-epoch optimization improves sample efficiency")
        
    else:
        print("Loading trained PPO model...")
        try:
            model = PPO.load("./pendulum/ppo/weights/ppo_pendulum", env=env)
