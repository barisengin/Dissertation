...
def create_comprehensive_ranking_visualization(df_ranked):
    ...
    plt.tight_layout()
    plt.savefig(f'./pendulum/ppo/cra/{env_name}_{alg_name}_cra.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def create_ranking_summary_table(df_ranked):
    ...
    print("="*100)
    print("COMPREHENSIVE CONFIGURATION RANKING ANALYSIS - PENDULUM PPO")
    print("="*100)
    print()
...
def identify_best_configuration(df_ranked):
    ...
    print("🏆 BEST PENDULUM PPO CONFIGURATION IDENTIFIED")
    print("="*49)
    print()
    ...
    print(f"   • Leverages PPO's clipped objective for smooth continuous control")
...
def generate_recommendations(df_ranked):
    ...
    print("\n" + "="*80)
    print("🎯 PENDULUM PPO OPTIMIZATION RECOMMENDATIONS FOR THESIS")
    print("="*80)
    print()
    ...
    print("2. KEY INSIGHTS FOR PENDULUM PPO OPTIMIZATION:")
    print("-" * 46)
    print("   • PPO's stability crucial for continuous control tasks")
    ...
    print("3. CRITICAL PPO SUCCESS FACTORS FOR PENDULUM:")
    print("-" * 43)
    print("   • Clip epsilon prevents overshooting in continuous action space")
    print("   • GAE lambda balances bias-variance for smooth control")
    print("   • Value loss coefficient crucial for accurate value estimation")
    print("   • Entropy coefficient maintains exploration in continuous domain")
    print()
    
    print("4. IMPLICATIONS FOR CONTINUOUS CONTROL ENVIRONMENTS:")
    print("-" * 54)
    print("   • PPO's stability advantages scale well to continuous tasks")
    print("   • Clipped objective particularly important for smooth control")
    print("   • Industry standard for continuous control applications")

def main():
    """Run comprehensive configuration ranking analysis"""
    
    json_file = f'./pendulum/ppo/json_data/{env_name}_{alg_name}_hp.json'
    
    try:
        # Load and process all configurations
        df = load_and_process_all_configs(json_file)
        
        print("📊 COMPREHENSIVE PENDULUM PPO CONFIGURATION RANKING ANALYSIS")
        print("="*64)
        ...
        print(f"\n✅ Pendulum PPO Analysis Complete!")
