...
class PerformanceLogger:
    ...
    def __init__(self, save_dir="./pendulum/a3c/json_data/"):
    ...
    def log_episode(self, episode_num, reward, episode_length, training_time, 
        ...
        # Log A3C-specific losses if provided
    ...
    def save_data(self, filename_prefix="a3c_pendulum"):
...
class TrainingCallback(BaseCallback):
    ...
    def _on_episode_end(self):
        ...
        if self.episode_start_time is not None:
            ...
            # Try to get A3C-specific loss information from model
            policy_loss = None
            value_loss = None
            entropy_loss = None
...
def run(is_training=True):
    print("Setting up Pendulum with A3C (A2C)...")
    
    # Create Pendulum environment with proper wrappers
    if is_training:
    ...
    # Create directories
    os.makedirs("./pendulum/a3c/weights/", exist_ok=True)
    os.makedirs("./pendulum/a3c/json_data/", exist_ok=True)
    ...
    if is_training:
        ...
        # A3C (A2C) configuration optimized for Pendulum continuous control
        model = A2C(
            "MlpPolicy",
            env,
            # Core A3C parameters optimized for continuous control
            learning_rate=7e-4,              # Standard A2C learning rate
            n_steps=5,                       # Short rollouts for fast updates
            ...
            policy_kwargs=dict(
                net_arch=[64, 64],           # Smaller network for simple Pendulum
        ...
        print("Model created with A3C (A2C) configuration for Pendulum:")
        print(f"- Algorithm: A2C (synchronous version of A3C)")
        print(f"- Parallel Environments: {env.num_envs}")
        print(f"- Actor-Critic Architecture: MLP + Continuous Policy")
        print(f"- Learning Rate: 7e-4")
        print(f"- N-steps: 5 (short rollouts for fast updates)")
        print(f"- Entropy Coefficient: 0.0 (dense rewards, no exploration bonus needed)")
        ...
        print("\nStarting training...")
        print("Note: Pendulum has dense rewards and should learn relatively quickly")
        print("Expected to see steady improvement in balance control")
        
        # Train the model (Pendulum typically needs fewer steps)
        total_timesteps = 1000000  # 100k steps should be sufficient for Pendulum
        ...
        # Save the model
        model.save("./pendulum/a3c/weights/a3c_pendulum")
        ...
        if logger.episode_rewards:
            ...
            # Entropy Loss (should be minimal since ent_coef=0.0)
            plt.subplot(3, 3, 7)
            ...
            plt.tight_layout()
            plt.savefig("./pendulum/a3c/weights/a3c_pendulum_training.png", 
                       dpi=300, bbox_inches='tight')
            plt.show()
            
            # Print final training summary
            final_metrics = logger.get_performance_metrics()
            print("\n=== A3C PENDULUM TRAINING SUMMARY ===")
            ...
            print(f"Dense reward environment - should see steady improvement")
            ...
            # A3C specific insights
            print(f"\nA3C Specific Metrics:")
            print(f"Parallel environments provide diverse experience")
            print(f"Short rollouts (n_steps=5) for fast policy updates")
            print(f"No entropy regularization (ent_coef=0.0) due to dense rewards")
            print(f"Actor-critic learning with advantage estimation")
        
    else:
        print("Loading trained A3C model...")
        try:
            model = A2C.load("./pendulum/a3c/weights/a3c_pendulum", env=env)
