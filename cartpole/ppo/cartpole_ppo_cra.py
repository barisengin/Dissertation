...
def create_comprehensive_ranking_visualization(df_ranked):
    ...
    plt.tight_layout()
    plt.savefig(f'./cartpole/ppo/cra/{env_name}_{alg_name}_cra.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def create_ranking_summary_table(df_ranked):
    ...
    print("="*100)
    print("COMPREHENSIVE CONFIGURATION RANKING ANALYSIS - PPO")
    print("="*100)
    print()
...
def identify_best_configuration(df_ranked):
    ...
    print("🏆 BEST PPO CONFIGURATION IDENTIFIED")
    print("="*40)
    print()
    ...
    print(f"   • Leverages PPO's clipped objective for stable learning")
...
def generate_recommendations(df_ranked):
    ...
    print("\n" + "="*80)
    print("🎯 PPO OPTIMIZATION RECOMMENDATIONS FOR THESIS")
    print("="*80)
    print()
    ...
    print("2. KEY INSIGHTS FOR PPO OPTIMIZATION:")
    print("-" * 37)
    print("   • PPO's clipped objective provides training stability")
    ...
    print("   • Balances policy improvement with stability constraints")
    print()
    
    print("3. CRITICAL PPO SUCCESS FACTORS:")
    print("-" * 32)
    print("   • Clip epsilon prevents destructive policy updates")
    print("   • GAE lambda controls bias-variance tradeoff")
    print("   • Value loss coefficient balances actor-critic learning")
    print("   • Entropy coefficient maintains exploration")
    print()
    
    print("4. IMPLICATIONS FOR COMPLEX ENVIRONMENTS:")
    print("-" * 45)
    print("   • PPO's stability advantages scale to complex tasks")
    print("   • Industry standard for continuous and discrete control")
    print("   • Foundation for understanding modern policy optimization")

def main():
    """Run comprehensive configuration ranking analysis"""
    
    json_file = f'./cartpole/ppo/json_data/{env_name}_{alg_name}_hp.json'
    
    try:
        # Load and process all configurations
        df = load_and_process_all_configs(json_file)
        
        print("📊 COMPREHENSIVE PPO CONFIGURATION RANKING ANALYSIS")
        print("="*57)
