...
def train_and_evaluate_single_seed(config, config_name, seed):
    ...
    try:
        ...
        return {
            'mean_episode_return': 0.0,  # Poor performance for CartPole
            'success_rate': 0.0,
...
def aggregate_seed_results(all_metrics, config, config_name):
    ...
    for metric in metrics_to_aggregate:
        ...
        if values:
            ...
            aggregated[f'{metric}_mean'] = 0.0  # Default poor performance for CartPole
...
def run_hyperparameter_tuning_with_stats(num_seeds=2):
    """Run systematic hyperparameter tuning with statistical testing for CartPole"""
    
    # Base configuration optimized for CartPole (simple discrete control)
    base_config = {
        ...
        'gae_lambda': 0.95,
        'clip_range': 0.2,
        'ent_coef': 0.0,  # Lower entropy for simple discrete task
        'vf_coef': 0.5,
        'max_grad_norm': 0.5,
        'policy_arch': [64, 64],
        'value_arch': [64, 64],
        'total_timesteps': 100000  # CartPole learns quickly
    }
    
    results = []
    
    # 1. Test different learning rates
    print("PHASE 1: Testing Learning Rates for CartPole PPO")
    ...
    # 2. Test different clip ranges with best learning rate
    print("\nPHASE 2: Testing Clip Ranges")
    ...
    try:
        ...
        with open(f'./cartpole/ppo/json_data/hyperparameter_results_stats_{timestamp}.json', 'w') as f:
    ...
    # Print summary
    print(f"\n{'='*60}")
    print("CARTPOLE PPO HYPERPARAMETER TUNING SUMMARY")
    print(f"{'='*60}")
    ...
    print(f"\nCartPole PPO hyperparameter tuning completed at {datetime.now()}")
...
if __name__ == "__main__":
    ...
    print("Starting CartPole PPO hyperparameter tuning with statistical significance testing...")
    print("Note: CartPole is a simple discrete control task that should learn quickly.")
    print("Expected training time: 30-60 minutes depending on hardware")
    
    try:
        results = run_hyperparameter_tuning_with_stats(num_seeds=2)
        print(f"✅ Successfully completed CartPole PPO hyperparameter tuning!")
