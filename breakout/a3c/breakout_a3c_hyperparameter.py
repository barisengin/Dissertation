#!/usr/bin/env python3
"""
Breakout A3C (A2C) Hyperparameter Tuning with Statistical Analysis
"""
...
def train_and_evaluate_single_seed(config, config_name, seed):
    ...
    try:
        ...
        env = make_atari_env('BreakoutNoFrameskip-v4', n_envs=config['n_envs'], seed=seed)
...
def run_hyperparameter_tuning_with_stats(num_seeds=2):
    """Run systematic hyperparameter tuning with statistical testing for Breakout A3C"""
    
    # Base configuration optimized for Atari A3C (A2C)
    base_config = {
        'learning_rate': 7e-4,
        'n_steps': 5,
        'gamma': 0.99,
    ...
    print("PHASE 1: Testing Learning Rates for Breakout A3C")
    ...
    n_envs_options = [8, 16, 32]  # Different numbers of parallel environments
    ...
    # 3. Test different entropy coefficients
    print("\nPHASE 3: Testing Entropy Coefficients")
    ...
    try:
        ...
        with open(f'./breakout/a3c/json_data/{env_name}_{alg_name}_hp.json', 'w') as f:
    ...
    # Print summary
    print(f"\n{'='*60}")
    print("BREAKOUT A3C HYPERPARAMETER TUNING SUMMARY")
    print(f"{'='*60}")
    ...
    print(f"\nBreakout A3C hyperparameter tuning completed at {datetime.now()}")
    print("Note: A3C benefits from parallel environments and is well-suited for Atari games")
...
if __name__ == "__main__":
    ...
    print("Starting Breakout A3C hyperparameter tuning with statistical significance testing...")
    print("Note: A3C uses parallel environments and is well-suited for Atari environments.")
    print("Expected training time: 3-6 hours depending on hardware")
    
    try:
        results = run_hyperparameter_tuning_with_stats(num_seeds=2)
        print(f"✅ Successfully completed Breakout A3C hyperparameter tuning!")
