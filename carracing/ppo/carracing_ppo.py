...
class PerformanceLogger:
    ...
    def __init__(self, save_dir="./carracing/ppo/json_data/"):
        ...
        # Performance metrics (CarRacing specific)
        self.convergence_threshold = 500.0  # CarRacing considered good performance at 500+
    ...
    def log_episode(self, episode_num, reward, episode_length, training_time, 
                   total_steps, policy_loss=None, value_loss=None, entropy_loss=None, clip_fraction=None):
    ...
    def save_data(self, filename_prefix="ppo_carracing"):
...
class CarRacingWrapper(gym.Wrapper):
    ...
    def reset(self, **kwargs):
        ...
        if isinstance(result, tuple):
            ...
            # Handle old gym API - return obs and empty info dict
            return result, {}
        
    def step(self, action):
        """Step environment and handle action format"""
        # Ensure action is properly formatted for CarRacing
        if isinstance(action, np.ndarray):
            action = action.astype(np.float32)
        
        obs, reward, terminated, truncated, info = self.env.step(action)
        
        # CarRacing specific reward shaping (optional)
        # Penalize for going off-track or being stationary
...
def run(is_training=True):
    print("Setting up CarRacing with PPO...")
    
    # Create CarRacing environment with proper wrappers
    if is_training:
        # Use fewer parallel environments for PPO (more sample efficient than A2C)
        def make_env():
    ...
    # Create directories
    os.makedirs("./carracing/ppo/weights/", exist_ok=True)
    ...
    print(f"Action dimensions: Steering [-1,1], Gas [0,1], Brake [0,1]")

    if is_training:
        logger = PerformanceLogger()
        
        # PPO configuration optimized for CarRacing continuous control
        model = PPO(
            "CnnPolicy",
            env,
            # Core PPO parameters optimized for continuous control
            ...
            # Network architecture for visual continuous control
            policy_kwargs=dict(
                net_arch=[256, 256],         # Larger network for complex visual-motor control
        ...
        print("Model created with PPO configuration for CarRacing:")
        print(f"- Algorithm: PPO (Proximal Policy Optimization)")
        print(f"- Parallel Environments: {env.num_envs}")
        print(f"- Policy Type: CNN + Continuous Actions")
        print(f"- Learning Rate: 3e-4")
        print(f"- N-steps: 2048 (steps per environment before update)")
        print(f"- Batch Size: 64")
        print(f"- N-epochs: 10 (optimization epochs per update)")
        print(f"- Clip Range: 0.2 (PPO clipping parameter)")
        print(f"- Entropy Coefficient: 0.01 (exploration)")
        print(f"- Value Function Coefficient: 0.5")
        print(f"- GAE Lambda: 0.95 (temporal credit assignment)")
        print(f"- Network Architecture: CNN + [256, 256] fully connected")
        print(f"- Action Space: Continuous [steering, gas, brake]")
        
        # Create callback for logging
        callback = TrainingCallback(logger, log_interval=5000)
        
        print("\nStarting training...")
        print("Note: PPO should be more sample efficient than A2C for CarRacing")
        print("Expected to see initial negative rewards before improvement")
        
        # Train the model (PPO typically needs fewer steps than A2C due to better sample efficiency)
        total_timesteps = 3000000  # 3 million steps should be sufficient for PPO
        ...
        # Save the model
        model.save("./carracing/ppo/weights/ppo_carracing")
        ...
        if logger.episode_rewards:
            ...
            if len(logger.episode_rewards) >= 50:
                ...
                plt.plot(episodes, moving_avg, color='red', linewidth=2, label='Moving Average (50 ep)')
            
            plt.axhline(y=500, color='green', linestyle='--', alpha=0.7, label='Good Performance (500)')
            ...
            plt.axvline(500, color='green', linestyle='--', label='Target: 500')
            ...
            if logger.episode_lengths:
                ...
                if len(logger.episode_lengths) >= 25:
                    moving_avg_len = []
                    ...
                    plt.plot(episodes, moving_avg_len, color='darkred', linewidth=2, label='Moving Avg (25 ep)')
                plt.xlabel('Episode')
            ...
            if logger.policy_losses:
                ...
                if len(logger.policy_losses) >= 10:
                    window = min(10, len(logger.policy_losses)//5)
                    smoothed_policy = []
            ...
            if logger.value_losses:
                ...
                if len(logger.value_losses) >= 10:
                    window = min(10, len(logger.value_losses)//5)
                    smoothed_value = []
            ...
            plt.subplot(3, 3, 7)
            if logger.clip_fractions:
                episodes_with_clip = range(len(logger.clip_fractions))
                plt.plot(episodes_with_clip, logger.clip_fractions, alpha=0.7, color='purple', label='Clip Fraction')
                if len(logger.clip_fractions) >= 10:
                    window = min(10, len(logger.clip_fractions)//5)
                    smoothed_clip = []
                    for i in range(window, len(logger.clip_fractions)):
                        smoothed_clip.append(np.mean(logger.clip_fractions[i-window:i]))
                    plt.plot(range(window, len(logger.clip_fractions)), smoothed_clip, 
                            color='darkviolet', linewidth=2, label=f'Smoothed ({window})')
                plt.axhline(y=0.2, color='red', linestyle='--', alpha=0.7, label='Clip Range (0.2)')
                plt.xlabel('Episode')
                plt.ylabel('Clip Fraction')
                plt.title('PPO Clip Fraction Over Time')
                plt.legend()
                plt.grid(True, alpha=0.3)
            ...
            if len(logger.episode_rewards) >= 50:
                ...
                plt.axhline(y=500, color='green', linestyle='--', alpha=0.7, label='Target')
                plt.axhline(y=0, color='black', linestyle='-', alpha=0.3, label='Break Even')
                plt.xlabel('Episode')
            ...
            plt.tight_layout()
            plt.savefig("./carracing/ppo/weights/ppo_carracing_training.png", 
                       dpi=300, bbox_inches='tight')
            plt.show()
            
            # Print final training summary
            final_metrics = logger.get_performance_metrics()
            print("\n=== PPO CARRACING TRAINING SUMMARY ===")
            ...
            if final_metrics['sample_efficiency']['threshold_achieved']:
                print(f"Threshold (500) achieved at episode: {final_metrics['sample_efficiency']['episodes_to_threshold']}")
            ...
            # CarRacing specific insights
            positive_rewards = [r for r in logger.episode_rewards if r > 0]
            negative_rewards = [r for r in logger.episode_rewards if r <= 0]
            print(f"\nCarRacing Specific Metrics:")
            print(f"Episodes with positive rewards: {len(positive_rewards)}/{len(logger.episode_rewards)} ({100*len(positive_rewards)/len(logger.episode_rewards):.1f}%)")
            if positive_rewards:
                print(f"Average positive reward: {np.mean(positive_rewards):.2f}")
            if negative_rewards:
                print(f"Average negative reward: {np.mean(negative_rewards):.2f}")
            
            # PPO specific insights
            if logger.clip_fractions:
                avg_clip_fraction = np.mean(logger.clip_fractions)
                print(f"\nPPO Specific Metrics:")
                print(f"Average clip fraction: {avg_clip_fraction:.3f}")
                if avg_clip_fraction < 0.05:
                    print("Low clip fraction suggests policy updates are conservative")
                elif avg_clip_fraction > 0.3:
                    print("High clip fraction suggests aggressive policy updates")
                else:
                    print("Clip fraction in good range for stable learning")
        
    else:
        print("Loading trained PPO model...")
        try:
            model = PPO.load("./carracing/ppo/weights/ppo_carracing", env=env)
            print("Model loaded successfully!")
            
            # Test the model
            print("Testing the PPO agent...")
            test_episodes = 5  # Fewer episodes since CarRacing episodes are longer
            test_rewards = []
