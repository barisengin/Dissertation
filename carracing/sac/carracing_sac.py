...
class PerformanceLogger:
    ...
    def __init__(self, save_dir="./carracing/sac/json_data/"):
    ...
    def save_data(self, filename_prefix="sac_carracing"):
...
def run(is_training=True):
    print("Setting up CarRacing with SAC...")
    ...
    # Create directories
    os.makedirs("./carracing/sac/weights/", exist_ok=True)
    os.makedirs("./carracing/sac/json_data/", exist_ok=True)
    ...
    if is_training:
        ...
        # SAC configuration optimized for CarRacing continuous control
        model = SAC(
            "CnnPolicy",
            env,
            # Core SAC parameters optimized for continuous control
            learning_rate=3e-4,              # Learning rate for all networks
            buffer_size=1000000,             # Large replay buffer for off-policy learning
            learning_starts=10000,           # Start training after collecting experience
            ...
            policy_kwargs=dict(
                net_arch=[256, 256],         # Network architecture for actor and critic
        ...
        print("Model created with SAC configuration for CarRacing:")
        print(f"- Algorithm: SAC (Soft Actor-Critic)")
        print(f"- Environments: {env.num_envs} (single env works well with replay buffer)")
        print(f"- Policy Type: CNN + Continuous Actions")
        print(f"- Learning Rate: 3e-4")
        print(f"- Buffer Size: 1,000,000 (large off-policy replay buffer)")
        print(f"- Batch Size: 256")
        print(f"- Learning Starts: 10,000 (warmup period)")
        ...
        print("\nStarting training...")
        print("Note: SAC should be very sample efficient for continuous control")
        print("Expected excellent exploration due to maximum entropy objective")
        
        # Train the model (SAC should be highly sample efficient)
        total_timesteps = 2000000  # 2 million steps should be sufficient for SAC
        ...
        # Save the model
        model.save("./carracing/sac/weights/sac_carracing")
        ...
        if logger.episode_rewards:
            ...
            plt.tight_layout()
            plt.savefig("./carracing/sac/weights/sac_carracing_training.png", 
                       dpi=300, bbox_inches='tight')
            plt.show()
            
            # Print final training summary
            final_metrics = logger.get_performance_metrics()
            print("\n=== SAC CARRACING TRAINING SUMMARY ===")
            ...
            if logger.entropy_coefficients:
                ...
                print("Entropy coefficient adaptation shows exploration-exploitation balance")
        
    else:
        print("Loading trained SAC model...")
        try:
            model = SAC.load("./carracing/sac/weights/sac_carracing", env=env)
