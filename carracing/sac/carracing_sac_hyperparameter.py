#!/usr/bin/env python3
"""
CarRacing SAC Hyperparameter Tuning with Statistical Analysis
"""
...
def train_and_evaluate_single_seed(config, config_name, seed):
    ...
    try:
        ...
        return {
            ...
            'total_episodes': config['total_timesteps'] // 1000,  # Approximate
...
def run_hyperparameter_tuning_with_stats(num_seeds=2):
    """Run systematic hyperparameter tuning with statistical testing for CarRacing SAC"""
    ...
    print("PHASE 1: Testing Learning Rates for CarRacing SAC")
    ...
    buffer_sizes = [100000, 200000, 500000]
    ...
    try:
        ...
        with open(f'./carracing/sac/json_data/{env_name}_{alg_name}_hp.json', 'w') as f:
    ...
    # Print summary
    print(f"\n{'='*60}")
    print("CARRACING SAC HYPERPARAMETER TUNING SUMMARY")
    print(f"{'='*60}")
    ...
    print(f"\nCarRacing SAC hyperparameter tuning completed at {datetime.now()}")
    print("Note: SAC is sample-efficient for continuous control but requires careful tuning")
...
if __name__ == "__main__":
    ...
    print("Starting CarRacing SAC hyperparameter tuning with statistical significance testing...")
    print("Note: SAC is well-suited for continuous control environments like CarRacing.")
    print("Expected training time: 2-4 hours depending on hardware")
    
    try:
        results = run_hyperparameter_tuning_with_stats(num_seeds=2)
        print(f"✅ Successfully completed CarRacing SAC hyperparameter tuning!")
