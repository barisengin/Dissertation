...
class PerformanceLogger:
    ...
    def __init__(self, save_dir="./carracing/a3c/json_data/"):
    ...
    def save_data(self, filename_prefix="a3c_carracing"):
...
def run(is_training=True):
    print("Setting up CarRacing with A3C (A2C)...")
    
    # Create CarRacing environment with proper wrappers
    if is_training:
        # Use multiple parallel environments for A3C
        def make_env():
        ...
        # Note: CarRacing doesn't need frame stacking like Atari
    else:
        def make_env():
            env = gym.make('CarRacing-v2', continuous=True)
            env = CarRacingWrapper(env)
    ...
    # Create directories
    os.makedirs("./carracing/a3c/weights/", exist_ok=True)
    ...
    if is_training:
        ...
        # A3C (A2C) configuration optimized for CarRacing continuous control
        model = A2C(
            "CnnPolicy",
            env,
            # Core A3C parameters optimized for continuous control
            learning_rate=3e-4,              # Lower learning rate for continuous control
            n_steps=128,                     # Longer rollouts for continuous control
        ...
        print("Model created with A3C (A2C) configuration for CarRacing:")
        print(f"- Algorithm: A2C (synchronous version of A3C)")
        print(f"- Parallel Environments: {env.num_envs}")
        print(f"- Actor-Critic Architecture: CNN + Continuous Policy")
        print(f"- Learning Rate: 3e-4 (optimized for continuous control)")
        print(f"- N-steps: 128 (longer rollouts for temporal dependencies)")
        print(f"- Entropy Coefficient: 0.01 (exploration in continuous space)")
        ...
        print("\nStarting training...")
        print("Note: CarRacing requires learning complex visual-motor coordination")
        print("Expected to see initial negative rewards before improvement")
        
        # Train the model (CarRacing typically needs substantial training)
        total_timesteps = 5000000  # 5 million steps for good CarRacing performance
        ...
        # Save the model
        model.save("./carracing/a3c/weights/a3c_carracing")
        ...
        if logger.episode_rewards:
            ...
            if logger.entropy_losses:
                ...
                if len(logger.entropy_losses) >= 10:
                    window = min(10, len(logger.entropy_losses)//5)
                    smoothed_entropy = []
            ...
            plt.tight_layout()
            plt.savefig("./carracing/a3c/weights/a3c_carracing_training.png", 
                       dpi=300, bbox_inches='tight')
            plt.show()
            
            # Print final training summary
            final_metrics = logger.get_performance_metrics()
            print("\n=== A3C CARRACING TRAINING SUMMARY ===")
        ...
        try:
            model = A2C.load("./carracing/a3c/weights/a3c_carracing", env=env)
