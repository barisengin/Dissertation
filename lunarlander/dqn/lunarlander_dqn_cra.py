...
Ranks all evaluated DQN configurations by the most significant metrics
"""
...
def create_comprehensive_ranking_visualization(df_ranked):
    ...
    plt.tight_layout()
    plt.savefig(f'./lunarlander/dqn/cra/{env_name}_{alg_name}_cra.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def create_ranking_summary_table(df_ranked):
    ...
    print("="*100)
    print("COMPREHENSIVE CONFIGURATION RANKING ANALYSIS - LUNARLANDER DQN")
    print("="*100)
    print()
...
def identify_best_configuration(df_ranked):
    ...
    print("🏆 BEST LUNARLANDER DQN CONFIGURATION IDENTIFIED")
    print("="*55)
    print()
    ...
    for key, value in config.items():
        if key in ['learning_rate', 'target_update', 'architecture', 'warmup_steps', 'train_interval', 'epsilon_min']:
            print(f"   • {key.replace('_', ' ').title()}: {value}")
    print()
    
    print("🎯 WHY THIS CONFIGURATION WINS:")
    ...
    print(f"   • Handles discrete action space with fuel management challenges")
...
def statistical_significance_of_winner(df_ranked):
    ...
    print("📈 STATISTICAL VALIDATION OF WINNER")
    print("="*50)
    print()
...
def generate_recommendations(df_ranked):
    ...
    print("\n" + "="*80)
    print("🎯 LUNARLANDER DQN OPTIMIZATION RECOMMENDATIONS FOR THESIS")
    print("="*80)
    print()
    ...
    if 'target_update' in config:
        print(f"   ✅ Target Update Rate: {config['target_update']}")
    if 'warmup_steps' in config:
        print(f"   ✅ Warmup Steps: {config['warmup_steps']}")
    if 'train_interval' in config:
        print(f"   ✅ Train Interval: {config['train_interval']}")
    
    print()
    
    print("2. KEY INSIGHTS FOR LUNARLANDER OPTIMIZATION:")
    print("-" * 50)
    print("   • Intermediate complexity requires balanced exploration-exploitation")
    ...
    print("   • Mixed reward structure demands careful hyperparameter tuning")
    print()
    
    print("3. CRITICAL SUCCESS FACTORS:")
    print("-" * 35)
    print("   • Learning rate crucial for converging in sparse reward environment")
    print("   • Target update frequency affects stability in multi-objective task")
    print("   • Warmup period essential for establishing good exploration patterns")
    print("   • Fuel management requires balanced action selection")
    print()
    
    print("4. IMPLICATIONS FOR COMPLEX ENVIRONMENTS:")
    print("-" * 45)
    print("   • LunarLander bridges simple control and complex visual tasks")
    print("   • Insights applicable to other intermediate complexity environments")
    print("   • Foundation for understanding discrete action space optimization")

def main():
    """Run comprehensive configuration ranking analysis"""
    
    json_file = f'./lunarlander/dqn/json_data/{env_name}_{alg_name}_hp.json'
    
    try:
        # Load and process all configurations
        df = load_and_process_all_configs(json_file)
        
        print("📊 COMPREHENSIVE LUNARLANDER DQN CONFIGURATION RANKING ANALYSIS")
        print("="*70)
        ...
        print(f"\n✅ LunarLander DQN Analysis Complete!")
