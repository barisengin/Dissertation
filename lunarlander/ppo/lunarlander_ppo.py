...
class PerformanceLogger:
    ...
    def __init__(self, save_dir="./lunarlander/ppo/json_data/"):
    ...
    def save_data(self, filename_prefix="ppo_lunarlander"):
...
class LunarLanderWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        # For example, penalize fuel usage more heavily or reward smooth landings
        # reward = reward  # Keep original reward structure for now
        
        return obs, reward, terminated, truncated, info

def run(is_training=True):
    print("Setting up LunarLander with PPO...")
    
    # Create LunarLander environment
    if is_training:
        # Use multiple parallel environments for better sample efficiency
        def make_env():
    ...
    # Create directories
    os.makedirs("./lunarlander/ppo/weights/", exist_ok=True)
    os.makedirs("./lunarlander/ppo/json_data/", exist_ok=True)
    ...
    if is_training:
        ...
        # PPO configuration optimized for LunarLander
        model = PPO(
            "MlpPolicy",
            env,
            # Core PPO parameters optimized for discrete control
            ...
            # Network architecture for LunarLander
            policy_kwargs=dict(
                net_arch=[256, 256],         # Larger network for intermediate complexity
        ...
        print("Model created with PPO configuration for LunarLander:")
        ...
        print(f"- GAE Lambda: 0.98 (temporal credit assignment)")
        print(f"- Network Architecture: [8] -> [256, 256] -> [4]")
        ...
        print("\nStarting training...")
        print("Note: PPO should learn LunarLander efficiently")
        print("Expected to see gradual improvement with stable learning")
        
        # Train the model
        total_timesteps = 1000000  # 1 million steps should be sufficient for LunarLander
        ...
        # Save the model
        model.save("./lunarlander/ppo/weights/ppo_lunarlander")
        ...
        if logger.episode_rewards:
            ...
            if logger.clip_fractions:
                ...
                if len(logger.clip_fractions) >= 20:
                    window = min(20, len(logger.clip_fractions)//5)
                    smoothed_clip = []
            ...
            plt.tight_layout()
            plt.savefig("./lunarlander/ppo/weights/ppo_lunarlander_training.png", 
                       dpi=300, bbox_inches='tight')
            plt.show()
            
            # Print final training summary
            final_metrics = logger.get_performance_metrics()
            print("\n=== PPO LUNARLANDER TRAINING SUMMARY ===")
        ...
        try:
            model = PPO.load("./lunarlander/ppo/weights/ppo_lunarlander", env=env)
