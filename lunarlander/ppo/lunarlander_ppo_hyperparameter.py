...
def train_and_evaluate_single_seed(config, config_name, seed):
    ...
    try:
        ...
        env = make_vec_env("LunarLander-v2", n_envs=1, seed=seed)
...
def run_hyperparameter_tuning_with_stats(num_seeds=2):
    ...
    # Base configuration optimized for LunarLander (more complex discrete control)
    base_config = {
        ...
        'gae_lambda': 0.95,
        'clip_range': 0.2,
        'ent_coef': 0.01,  # More exploration for LunarLander
        'vf_coef': 0.5,
        'max_grad_norm': 0.5,
        'policy_arch': [128, 128],  # Larger network for complex environment
        'value_arch': [128, 128],
        'total_timesteps': 300000  # More timesteps for complex environment
    }
    
    results = []
    
    # 1. Test different learning rates
    print("PHASE 1: Testing Learning Rates for LunarLander PPO")
    ...
    ent_coefs = [0.0, 0.01, 0.05]  # LunarLander benefits from exploration
    ...
    try:
        ...
        with open(f'./lunarlander/ppo/json_data/hyperparameter_results_stats_{timestamp}.json', 'w') as f:
    ...
    # Print summary
    print(f"\n{'='*60}")
    print("LUNARLANDER PPO HYPERPARAMETER TUNING SUMMARY")
    print(f"{'='*60}")
    ...
    print(f"\nLunarLander PPO hyperparameter tuning completed at {datetime.now()}")
...
if __name__ == "__main__":
    ...
    print("Starting LunarLander PPO hyperparameter tuning with statistical significance testing...")
    print("Note: LunarLander is more complex than CartPole and requires more exploration.")
    print("Expected training time: 1-2 hours depending on hardware")
    
    try:
        results = run_hyperparameter_tuning_with_stats(num_seeds=2)
        print(f"✅ Successfully completed LunarLander PPO hyperparameter tuning!")
