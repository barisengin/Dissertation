...
Ranks all evaluated Rainbow DQN configurations by the most significant metrics
"""
...
def load_and_process_all_configs(json_file):
    ...
    for result in results:
        ...
        individual_success = [1 if seed['test_mean_reward'] >= 200 else 0 for seed in result['seed_results']]  # LunarLander success threshold
        individual_efficiency = [seed.get('sample_efficiency_150') for seed in result['seed_results']]  # LunarLander efficiency threshold
        ...
        config_data = {
            ...
            'sample_efficiency': result.get('sample_efficiency_150_mean'),
            'sample_efficiency_success_rate': result.get('sample_efficiency_150_success_rate', 0),
            ...
            # Derived Metrics
            'solved': result['test_mean_reward_mean'] >= 200,  # LunarLander solved threshold
...
def calculate_comprehensive_rankings(df):
    ...
    # Weights based on importance: Performance (40%), Success Rate (30%), Stability (20%), Efficiency (10%)
    weights = {
        'performance': 0.4,
        'success_rate': 0.3,
        'stability': 0.2,
...
def create_comprehensive_ranking_visualization(df_ranked):
    ...
    plt.axvline(x=200, color='blue', linestyle='--', alpha=0.8, linewidth=2, label='Solved Threshold')
    ...
    plt.title('(C) Stability Ranking\n(Rainbow Enhancement)', fontweight='bold')
    plt.grid(axis='x', alpha=0.3)
    ...
    if not efficient_configs.empty:
        ...
        plt.yticks(range(len(plot_data)), plot_data['config_name'])
        plt.xlabel('Episodes to Reach 150+ Reward')
        plt.title('(D) Sample Efficiency Ranking\n(Rainbow Advantage)', fontweight='bold')
        plt.grid(axis='x', alpha=0.3)
    ...
    plt.title('(E) Overall Ranking\n(Multi-Component)', fontweight='bold')
    plt.grid(axis='x', alpha=0.3)
    ...
    plt.axhline(y=200, color='blue', linestyle='--', alpha=0.8, linewidth=2)
    plt.grid(axis='y', alpha=0.3)
    
    plt.tight_layout()
    plt.savefig(f'./lunarlander/rainbow/cra/{env_name}_{alg_name}_cra.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def create_ranking_summary_table(df_ranked):
    ...
    print("="*100)
    print("COMPREHENSIVE CONFIGURATION RANKING ANALYSIS - LUNARLANDER RAINBOW DQN")
    print("="*100)
    print()
...
def identify_best_configuration(df_ranked):
    ...
    print("🏆 BEST LUNARLANDER RAINBOW DQN CONFIGURATION IDENTIFIED")
    print("="*62)
    print()
    ...
    print(f"   • Solved LunarLander: {'✅ YES' if best_config['solved'] else '❌ NO'}")
    print()
    
    print("⚙️ RAINBOW DQN CONFIGURATION DETAILS:")
    config = best_config['full_config']
    rainbow_params = ['learning_rate', 'multi_step', 'prioritized_replay', 'noisy_nets', 'dueling', 'double_q']
    for key, value in config.items():
        if key in rainbow_params or key in ['architecture', 'warmup_steps', 'train_interval']:
            print(f"   • {key.replace('_', ' ').title()}: {value}")
    print()
    
    print("🎯 WHY THIS RAINBOW DQN CONFIGURATION WINS:")
    ...
    print(f"   • {'Solves' if best_config['solved'] else 'Approaches solution for'} LunarLander environment")
    print(f"   • Leverages Rainbow's enhanced features for complex landing task")
    
    if pd.notna(best_config['sample_efficiency']):
        print(f"   • Learns efficient landing strategy in {best_config['sample_efficiency']:.0f} episodes")
...
def statistical_significance_of_winner(df_ranked):
    ...
    print("📈 STATISTICAL VALIDATION OF RAINBOW DQN WINNER")
    print("="*52)
    print()
...
def generate_recommendations(df_ranked):
    ...
    print("\n" + "="*80)
    print("🎯 LUNARLANDER RAINBOW DQN OPTIMIZATION RECOMMENDATIONS FOR THESIS")
    print("="*80)
    print()
    
    best_config = df_ranked.iloc[0]
    
    print("1. OPTIMAL RAINBOW DQN HYPERPARAMETER VALUES:")
    print("-" * 45)
    config = best_config['full_config']
    
    rainbow_params = ['learning_rate', 'multi_step', 'prioritized_replay', 'noisy_nets', 'dueling', 'double_q']
    for param in rainbow_params:
        if param in config:
            print(f"   ✅ {param.replace('_', ' ').title()}: {config[param]}")
    
    print()
    
    print("2. KEY INSIGHTS FOR LUNARLANDER RAINBOW DQN OPTIMIZATION:")
    print("-" * 58)
    print("   • Rainbow's multi-component design excels at complex landing tasks")
    ...
    print("   • Component synergies crucial for multi-objective lunar landing")
    print()
    
    print("3. CRITICAL RAINBOW DQN SUCCESS FACTORS FOR LUNARLANDER:")
    print("-" * 55)
    print("   • Multi-step returns capture long-term landing trajectory rewards")
    print("   • Prioritized replay focuses on difficult landing scenarios")
    print("   • Noisy networks aid exploration of diverse landing strategies")
    print("   • Dueling architecture separates state value from action advantages")
    print()
    
    print("4. IMPLICATIONS FOR INTERMEDIATE COMPLEXITY ENVIRONMENTS:")
    print("-" * 58)
    print("   • Rainbow's enhancements particularly beneficial for multi-objective tasks")
    print("   • Component interactions require careful tuning in complex environments")
    print("   • Strong foundation for scaling to more complex control problems")

def main():
    """Run comprehensive configuration ranking analysis"""
    
    json_file = f'./lunarlander/rainbow/json_data/{env_name}_{alg_name}_hp.json'
    
    try:
        # Load and process all configurations
        df = load_and_process_all_configs(json_file)
        
        print("📊 COMPREHENSIVE LUNARLANDER RAINBOW DQN CONFIGURATION RANKING ANALYSIS")
        print("="*76)
        ...
        print(f"\n✅ LunarLander Rainbow DQN Analysis Complete!")
